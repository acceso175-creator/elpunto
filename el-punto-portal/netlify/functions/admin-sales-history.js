...
const addOrder = (stats, order) => {
  ...
  if (isActiveSale(order)) stats.salesTotal += total;
...
export async function handler(event) {
  try {
    ...
    orders.forEach((order) => {
      ...
      const collected = isCollected(order); const courtesy = isActiveSale(order) && order.payment_method === 'cortesia'; if (!isActiveSale(order)) return;
      (order.admin_order_items || []).forEach((item) => { if (!productMap.has(item.product_name)) productMap.set(item.product_name, { product: item.product_name, quantity: 0, totalSold: 0, averageTicket: 0, courtesies: 0 }); const row = productMap.get(item.product_name); if (collected) { row.quantity += Number(item.quantity || 0); row.totalSold += Number(item.total_price || 0); } if (courtesy) row.courtesies += Number(item.quantity || 0); row.averageTicket = row.quantity ? money(row.totalSold / row.quantity) : 0; });
      const cap = order.captured_by || 'Sin capturista'; if (!capturerMap.has(cap)) capturerMap.set(cap, { capturedBy: cap, ...emptyStats() }); addOrder(capturerMap.get(cap), order);
    });
    return json(200, { timezone: TZ, summary: sanitize(summary), grouped: [...grouped.values()].sort((a,b) => a.period.localeCompare(b.period)).map(sanitize), orders, products: [...productMap.values()].sort((a,b) => b.quantity - a.quantity).map(sanitize), capturers: [...capturerMap.values()].sort((a,b) => b.paymentsCollected - a.paymentsCollected).map(sanitize) });
