...
function capturedByName(order) { return order?.captured_by_name || order?.captured_by || 'Sin identificar'; }
...
function ReceiptTicket({ order, className = '' }) {
  ...
  return <div className={`receipt-ticket ${className}`.trim()}>
    ...
    <div className="receipt-ticket__meta">{[order.customer_name && `Cliente: ${order.customer_name}`, order.order_type && `Tipo: ${order.order_type}`, order.payment_method && `Pago: ${paymentLabel(order.payment_method)}`, order.status && `Estado: ${order.status}`, `Capturado por: ${capturedByName(order)}`].filter(Boolean).map((line) => <span key={line}>{line}</span>)}</div>
...
function receiptPdfLines(order) {
  ...
  lines.push({ sep: true });
  [order.customer_name && `Cliente: ${order.customer_name}`, order.customer_phone && `Teléfono: ${order.customer_phone}`, order.delivery_address && `Dirección: ${order.delivery_address}`, order.order_type && `Tipo: ${order.order_type}`, order.payment_method && `Pago: ${paymentLabel(order.payment_method)}`, order.status && `Estado: ${order.status}`, `Capturado por: ${capturedByName(order)}`].filter(Boolean).forEach((text) => lines.push({ text }));
  lines.push({ sep: true });
...
function receiptPdfLayout(order) {
  ...
  pushSeparator();
  [order.customer_name && `Cliente: ${order.customer_name}`, order.customer_phone && `Teléfono: ${order.customer_phone}`, order.delivery_address && `Dirección: ${order.delivery_address}`, order.order_type && `Tipo: ${order.order_type}`, order.payment_method && `Pago: ${paymentLabel(order.payment_method)}`, order.status && `Estado: ${order.status}`, `Capturado por: ${capturedByName(order)}`].filter(Boolean).forEach((text) => pushText(text, { size: 10.5, gap: 4.7 }));
  pushSeparator();
...
function OrderAccordionRow({ order, columns, expanded, onToggle, onSaved, onCancel }) {
  ...
  const expandedContent = <div className="order-accordion"><div className="order-detail-grid"><section><h4>Encabezado</h4>{[order.order_number, order.created_at && new Date(order.created_at).toLocaleString('es-MX'), orderEdited(order) && 'Editado', order.status && `Estado: ${order.status}`, order.payment_method && `Pago: ${paymentLabel(order.payment_method)}`, platformText(order) && `Plataforma: ${platformText(order)}`].filter(Boolean).map((line) => <p key={line}>{line}</p>)}</section><section><h4>Cliente</h4>{[order.customer_name && `Nombre: ${order.customer_name}`, `Teléfono: ${order.customer_phone || 'No registrado'}`, order.order_type && `Entrega: ${order.order_type}`, order.delivery_address && `Dirección: ${order.delivery_address}`].filter(Boolean).map((line) => <p key={line}>{line}</p>)}<div className="phone-inline-editor">{phoneEditing ? <><input type="tel" inputMode="tel" autoComplete="tel" placeholder="614 123 4567" value={phoneDraft} onChange={(event) => setPhoneDraft(event.target.value)} disabled={saving} /><button type="button" onClick={savePhone} disabled={saving}>{saving ? 'Guardando...' : 'Guardar'}</button><button type="button" className="button--ghost" onClick={() => { setPhoneDraft(order.customer_phone || ''); setPhoneEditing(false); setPhoneStatus(''); }} disabled={saving}>Cancelar</button></> : <button type="button" className="button--ghost" onClick={() => setPhoneEditing(true)}>{order.customer_phone ? 'Editar teléfono' : 'Agregar teléfono'}</button>}{phoneStatus && <small>{phoneStatus}</small>}</div></section></div>
    ...
    <section className="order-detail-extra">{[order.notes && `Notas: ${order.notes}`, `Capturó: ${capturedByName(order)}`, order.paid_at && `Pagado: ${new Date(order.paid_at).toLocaleString('es-MX')}`, latestEditReason(order) && `Edición: ${latestEditReason(order)}`].filter(Boolean).map((line) => <p key={line}>{line}</p>)}</section>
...
function SalesHistory({ onBack }) {
  ...
  function exportCsv() {
    if (!data) return; const lines = [['Resumen'], ...Object.entries(data.summary).map(([k, v]) => [k, v]), [], ['Ventas agrupadas'], ['Periodo','Total vendido','Efectivo','Tarjeta','Transferencia','Cortesías','Pedidos pagados','Pedidos cancelados','Ticket promedio'], ...data.grouped.map((r) => [r.period,salesTotalValue(r),r.efectivo,r.tarjeta,r.transferencia,r.cortesia,r.paidOrders,r.canceledOrders,r.averageTicket]), [], ['Pedidos'], ['Fecha','Folio','Cliente','Total','Pago','Tipo','Estado','Capturado por'], ...data.orders.map((o) => [o.created_at,o.order_number,o.customer_name,o.total,o.payment_method,o.order_type,o.status,capturedByName(o)]), [], ['Productos'], ['Producto','Cantidad','Total vendido','Ticket promedio','Cortesías'], ...data.products.map((p) => [p.product,p.quantity,salesTotalValue(p),p.averageTicket,p.courtesies]), [], ['Capturistas'], ['Capturista','Total','Pedidos','Ticket','Efectivo','Tarjeta','Transferencia','Cortesías','Cancelados'], ...data.capturers.map((c) => [c.capturedBy,salesTotalValue(c),c.paidOrders,c.averageTicket,c.efectivo,c.tarjeta,c.transferencia,c.cortesia,c.canceledOrders])];
...
function AdminSection({ menu, setMenu, business, setBusiness, productImages, refreshProductImages, productImagesError, dataSource, setDataSource }) {
  ...
  async function loadOptionTemplates() {
    if (!isSupabaseConfigured) return;
    try { const result = await adminRequest('admin-option-templates', { method: 'GET' }); setOptionTemplates(result.templates || []); setTemplatesStatus('Plantillas sincronizadas.'); }
    catch (error) { setTemplatesStatus(error.message); }
  }

  async function saveOptionTemplate(template, method = 'PATCH') {
    try { const result = await adminRequest('admin-option-templates', { method, body: method === 'POST' ? { template } : { ...template, items: template.items } }); setOptionTemplates(result.templates || []); setTemplatesStatus('Plantilla guardada.'); }
    catch (error) { setTemplatesStatus(error.message); }
  }

  async function deleteOptionTemplate(template) {
    const really = template.usageCount > 0 ? confirm(`Esta plantilla se usa en ${template.usageCount} producto(s). ¿Eliminar de todos modos?`) : confirm('¿Eliminar plantilla?');
    if (!really) return;
    try { const result = await adminRequest('admin-option-templates', { method: 'DELETE', body: { id: template.id, confirm: template.usageCount > 0 } }); setOptionTemplates(result.templates || []); setTemplatesStatus('Plantilla eliminada.'); }
    catch (error) { setTemplatesStatus(error.message); }
  }
  ...
  return (
    <section id="admin" className="section admin-grid">
      ...
      <OptionTemplatesLibrary templates={optionTemplates} status={templatesStatus} onReload={loadOptionTemplates} onSave={saveOptionTemplate} onDelete={deleteOptionTemplate} />
              ...
              <div key={category.id} className="admin-category">
                ...
                {category.items.map((item) => (
                  <AdminProductEditor
                    ...
                    onTemplatesChanged={async () => { await loadOptionTemplates(); await reloadAdminMenu(); }}
...
function OptionTemplatesLibrary({ templates, status, onReload, onSave, onDelete }) {
  ...
  return <div className="panel admin-full"><div className="admin-header"><div><p className="eyebrow">Admin</p><h2>Plantillas de opciones</h2></div><div className="admin-actions"><button type="button" className="button--ghost" onClick={onReload}>Recargar</button><button type="button" onClick={() => setDraft(blank())}>Nueva plantilla</button></div></div>
    ...
    <div className="option-group-editor"><div className="option-group-editor__grid">
      ...
      <label>Mínimo<input type="number" min="0" value={draft.minSelect ?? 0} onChange={(e) => setDraft({ ...draft, minSelect: Math.max(draft.required ? 1 : 0, Number(e.target.value) || 0) })} /></label>
      <label>Máximo<input type="number" min="1" disabled={draft.selectionType !== 'multiple'} value={draft.selectionType === 'multiple' ? draft.maxSelect ?? 1 : 1} onChange={(e) => setDraft({ ...draft, maxSelect: Math.max(1, draft.minSelect || 0, Number(e.target.value) || 1) })} /></label>
...
function AdminProductEditor({ item, category, adminCategories, productImages, optionTemplates, onTemplatesChanged, persistProduct, deleteItem, refreshProductImages, productImagesError }) {
